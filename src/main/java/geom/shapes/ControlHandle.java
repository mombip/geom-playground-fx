...
//        double result1 = (Math.abs(arm1VectNorm.getX()) - Math.abs(arm2VectNorm.getX()));
//        double result2 = (Math.abs(arm1VectNorm.getY()) - Math.abs(arm2VectNorm.getY()));
//        System.out.println(String.format("angle: %.5f v1:%s  v2:%s", Math.abs(result1) + Math.abs(result2),
//                arm1VectNorm, arm2VectNorm));
//        System.out.println(
//                String.format("angle: %.5f v1:%s  v2:%s", Math.abs(result1) + Math.abs(result2), arm1Vect, arm2Vect));

        return handleDirection;

        //        final double a1Length = arm1Vect.magnitude();
        //        final double a2Length = arm2Vect.magnitude();
        //        //        final double baseLength = arm1Vect.distance(arm2Vect);
        //        final double sum = a1Length + a2Length;
        //        final Point2D a1 = arm1VectNorm.multiply(a1Length / sum);
        //        final Point2D a2 = arm2VectNorm.multiply(a2Length / sum);
        //        final Point2D handleDirection = a1.subtract(a2).normalize();
        //        return handleDirection;
